...
import { Send, Calendar, Car, Briefcase, DollarSign, ArrowLeft } from 'lucide-react';
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: `G'day! 👋 I'm Nik Finance's AI lending specialist for **Asset, Business & Personal Finance**.

I can help you with:
🚗 **Car Finance** — new, used, refinancing
💼 **Business Loans** — equipment, working capital, expansion
💰 **Personal Loans** — debt consolidation, major purchases
...
What can I help you with today?`,
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: `[Context: User is asking about ASSET, BUSINESS, or PERSONAL finance - car loans, business loans, personal loans. Focus advice on these areas.]\n\n${text}`,
          threadId
        })
      });
  ...
  const handleLeadSubmit = async () => {
    ...
    setMessages(prev => [...prev, {
      id: (Date.now() + 1).toString(),
      role: 'assistant',
      content: `Thanks${leadName ? ` ${leadName}` : ''}! 🎉 Our team will reach out to you at ${leadEmail} shortly. In the meantime, feel free to keep chatting or book a consultation!`,
      timestamp: new Date()
    }]);
  ...
  return (
    <>
      <SEO 
        title="AI Car & Business Loan Assistant | Nik Finance"
        description="Chat with our AI lending specialist about car finance, business loans, and personal loans. Get instant answers and personalised advice."
      />
      <Header />
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-heading font-bold text-primary mb-4">
              Asset, Business & Personal Finance
            </h1>
            <div className="flex justify-center gap-6 text-sm text-gray-600">
              <span className="flex items-center gap-2"><Car className="w-5 h-5 text-accent" /> Car Loans</span>
              <span className="flex items-center gap-2"><Briefcase className="w-5 h-5 text-accent" /> Business</span>
              <span className="flex items-center gap-2"><DollarSign className="w-5 h-5 text-accent" /> Personal</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <div className="flex gap-3">
                <Input
                  type="text"
                  placeholder="Ask about car loans, business finance, personal loans..."
                  value={inputValue}
